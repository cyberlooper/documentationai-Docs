...
#### 1. "Linear" Mode
- **Calculated Punishment**: The duration increases mathematically for each offense.
- **Formulas**:
  - `Multiplier`: Doubles (or triples, etc.) the time each offense (30m -> 1h -> 2h).
  - `Increment`: Adds a fixed time each offense (30m -> 1h -> 1h30m).
- **Caps**: You can set a `max-duration` so it never exceeds, say, 24 hours.

#### 2. "Fractal" (Ladder) Mode
For specific rules (like "Swearing"), you can set a custom **ladder** of punishments:
